    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.existingSubmission) {
          setExistingSubmission(response.existingSubmission);
          if (response.existingSubmission.githubUrl) {
            setRepoName(response.existingSubmission.githubUrl);
          }
          if (response.existingSubmission.notes) {
            setCompletionNotes(response.existingSubmission.notes);
          }
        }
      }
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="border-t pt-6">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    <CheckCircle size={20} />
                    {existingSubmission?.status === 'submitted' ? 'Update Report' : 'Report Completion'}
                  </button>
                  <button
        ...
        {showCompletionForm && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative">
              <button
                onClick={() => {
                  setShowCompletionForm(false);
                  setCompletionNotes('');
              ...
              <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-6">
                {existingSubmission?.status === 'submitted' ? 'Update Completion Report' : 'Report Task Completion'}
              </h3>
            {existingSubmission?.status === 'submitted' && (
              <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 rounded text-sm flex items-start gap-2">
                ...
                <span>You have already submitted a report for this task. Do you want to change it? Submitting again will update your existing report.</span>
              </div>
            )}
            <form onSubmit={handleTaskCompletion} className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isSubmitting}
                >
                  {isSubmitting ? 'Submitting...' : (existingSubmission?.status === 'submitted' ? 'Update Report' : 'Submit Completion Report')}
                </button>
                <button
                  type="button"
                  onClick={() => {
